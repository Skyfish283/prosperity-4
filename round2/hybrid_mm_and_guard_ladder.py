...
# best params from just pluging and playing
DEFAULT_OSMIUM_STRATEGY_PARAMS: Dict[str, Any] = {
    "aggressive_threshold": 10.0,  # Switch to aggression when spread is this wide
    "safe_spread": 0.5,  # Minimum spread in safe mode
    "edge": 1,  # Undercut amount
    "inventory_skew_coeff": 3.0,  # How hard to lean when inventory is full
    "volatility_spread_coeff": 2.0,
}
...
class MarketMakingStrategy(Strategy):
    def __init__(
        ...
        self.mid_price_history: Deque[Tuple[int, float]] = deque(
            maxlen=self.volatility_lookback
        )
    ...
    def get_quotes(
        self, true_value: float, position: int, volatility: float
    ) -> Tuple[int, int]:
    ...
    def act(self, state: TradingState) -> None:
        ...
        observed_mid_price = self.get_observed_mid_price(
            true_value, buy_orders, sell_orders
        )
    ...
    def load(self, data) -> None:
        ...
        self.mid_price_history.extend(
            ...
            for timestamp, mid_price in data.get("mid_price_history", [])[
...
class OsmiumStrategy(MarketMakingStrategy):
    def __init__(
        ...
        symbol: Symbol,
        limit: int,
        aggressive_threshold: float = 8.0,
        safe_spread: float = 2.0,
        edge: int = 1,
        **kwargs,
    ) -> None:
        super().__init__(symbol, limit, base_spread=safe_spread, **kwargs)
        self.aggressive_threshold = aggressive_threshold
        self.edge = edge
        self.latest_depth = None

    def get_true_value(self, state: TradingState) -> int:
        # Use the Osmium-style 10k anchor logic for our "Safe" anchor
        order_depth = state.order_depths.get(self.symbol)
        if not order_depth or not order_depth.buy_orders or not order_depth.sell_orders:
            return 10000

        current_mid = (max(order_depth.buy_orders) + min(order_depth.sell_orders)) / 2
        # Anchor toward 10,000 to prevent chasing "fake" price spikes
        return round(10000 + 0.5 * (current_mid - 10000))

    def get_quotes(
        self, true_value: float, position: int, volatility: float
    ) -> Tuple[int, int]:
        if (
            not self.latest_depth
            or not self.latest_depth.buy_orders
            or not self.latest_depth.sell_orders
        ):
            return super().get_quotes(true_value, position, volatility)

        best_bid = max(self.latest_depth.buy_orders.keys())
        best_ask = min(self.latest_depth.sell_orders.keys())
        current_spread = best_ask - best_bid

        # --- THE AGGRESSION SWITCH ---
        # If the spread is wide, we go into "Aggressive Mode" to steal the top of the book.
        if current_spread >= self.aggressive_threshold:
            bid_quote = best_bid + self.edge
            ask_quote = best_ask - self.edge

        # --- THE SAFE MODE ---
        # If the spread is thin, we play it safe and use our theoretical "Reservation Price".
        else:
            inv_fraction = position / self.limit if self.limit else 0
            reservation_price = true_value - (self.inventory_skew_coeff * inv_fraction)

            # We place our quotes slightly outside the mid-price to ensure profit
            bid_quote = math.floor(reservation_price - (self.base_spread / 2))
            ask_quote = math.ceil(reservation_price + (self.base_spread / 2))

        # --- SAFETY OVERRIDE ---
        # Never allow the bid to be higher than the ask
        if bid_quote >= ask_quote:
            bid_quote = math.floor(true_value - 1)
            ask_quote = math.ceil(true_value + 1)

        return int(bid_quote), int(ask_quote)

    def act(self, state: TradingState) -> None:
        self.latest_depth = state.order_depths.get(self.symbol)
        if (
            not self.latest_depth
            or not self.latest_depth.buy_orders
            or not self.latest_depth.sell_orders
...
class PepperStrategy(Strategy):
    def __init__(
        ...
        normal_buy_levels: Sequence[Tuple[int, int]] = (
            (0, 4),
            (-1, 6),
            (-2, 8),
            (-3, 10),
        ...
        normal_sell_levels: Sequence[Tuple[int, int]] = (
            (2, 4),
            (3, 6),
            (4, 8),
            (5, 10),
    ...
    def fair_price(self, state: TradingState) -> float:
        return self.fair_price_base + self.fair_price_slope * (
            state.timestamp / 1_000_000
        )
        # # Step 1: Try to look at the actual order book
        # order_depth = state.order_depths.get(self.symbol)
        #
        # # Step 2: If the book is empty, fallback to the old hardcoded math
        # if not order_depth or not order_depth.buy_orders or not order_depth.sell_orders:
        #     return self.fair_price_base + self.fair_price_slope * (
        #         state.timestamp / 1_000_000
        #     )
        #
        # # Step 3: Calculate the current mid-price
        # best_bid = max(order_depth.buy_orders.keys())
        # best_ask = min(order_depth.sell_orders.keys())
        # current_mid = (best_bid + best_ask) / 2.0
        #
        # # Step 4: Return the dynamic mid-price as the anchor for your ladders
        # return current_mid

    def place_buy_ladder(
        self, levels: Sequence[Tuple[int, int]], remaining: int
    ) -> None:
    ...
    def act(self, state: TradingState) -> None:
        ...
        if state.timestamp >= self.close_start_timestamp:
            ...
            end_timestamp = (
                self.close_start_timestamp + self.close_steps * self.close_step_size
            )
            steps_remaining = max(
                1.0, (end_timestamp - state.timestamp) / self.close_step_size
            )
            expected_passive_close = (
                self.close_passive_volume_per_step * steps_remaining
            )
            ...
            if position > 0:
                base_ask = int(
                    fair
                    + self.close_passive_offset
                    - aggression * self.close_aggression_ticks
                )
                must_close_now = max(0, int(position - expected_passive_close))
                close_sizes = (
                    max(must_close_now, self.close_front_level_size),
                    *self.close_extra_level_sizes,
                )
                sell_levels = [
                    (base_ask + offset, quantity)
                    for offset, quantity in zip((0, 1, 2), close_sizes)
                ]
                ...
                buy_levels = [
                    (base_bid + offset, quantity)
                    for offset, quantity in zip((0, -1, -2), close_sizes)
                ]
                self.place_buy_ladder(buy_levels, -position)
            return

        # --- FETCH DATA FOR GUARDS ---
        order_depth = state.order_depths.get(self.symbol)
        best_bid = (
        ...
        # --- BUY LADDER WITH GUARDS ---
        buy_capacity = self.limit - position
        buy_levels = []
        for offset, quantity in self.normal_buy_levels:
            price = int(fair) + offset
            if best_bid is not None:
                price = min(price, best_bid + 1)  # Guard: Don't bid too high
            buy_levels.append((price, quantity))
        self.place_buy_ladder(buy_levels, buy_capacity)

        # --- SELL LADDER WITH GUARDS ---
        sell_capacity = self.limit + position
        sell_levels = []
        for offset, quantity in self.normal_sell_levels:
            price = int(fair) + offset
            if best_ask is not None:
                price = max(price, best_ask - 1)  # Guard: Don't ask too low
            sell_levels.append((price, quantity))
...
class Trader:
    def __init__(
        ...
        ash_params = {
            **DEFAULT_OSMIUM_STRATEGY_PARAMS,
            **strategy_params.get(ASH_COATED_OSMIUM, {}),
        }

        pepper_params = {
            **DEFAULT_PEPPER_STRATEGY_PARAMS,
            **strategy_params.get(INTARIAN_PEPPER_ROOT, {}),
        }

        self.strategies = {
            ASH_COATED_OSMIUM: OsmiumStrategy(
                ASH_COATED_OSMIUM, limits[ASH_COATED_OSMIUM], **ash_params
            ),
            INTARIAN_PEPPER_ROOT: PepperStrategy(
                INTARIAN_PEPPER_ROOT, limits[INTARIAN_PEPPER_ROOT], **pepper_params
            ),
        }
