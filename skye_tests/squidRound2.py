...
class meanReversionStrategy(Strategy):
    def __init__(self, symbol: Symbol, limit: int,trend_decay: float = 0.5) -> None:
        super().__init__(symbol, limit)
        self.history = deque() 
    ...
    def get_true_value(self,state: TradingState) -> int:
        ...
        # calculate the moving mean value using past 10 data points
        prices = np.array(self.history)
        weights = np.array([self.trend_decay**i for i in reversed(range(len(prices)))])
        true_value = np.sum(prices * weights) / np.sum(weights)
        return true_value
    ...
    def act(self, state: TradingState) -> None:
        ...
        # Calculate volatility-based dynamic threshold
        price_changes = np.diff(self.history) 
        volatility = np.std(price_changes)  
        threshold = max(2, volatility*2)
        ...
        # --BUY---
        for ask_price, volume in sell_orders:
            if ask_price < true_value -  threshold and to_buy > 0:
                qty = min(to_buy, -volume)
                self.buy(ask_price, qty)
                to_buy -= qty
                print(f"BUY triggered at {ask_price} (True value: {true_value}, threshold: {threshold})")
            if to_buy <= 0:
                break

        #---SELL---
        for bid_price, volume in buy_orders:
            if bid_price > true_value + 2 * threshold and to_sell > 0:
                qty = min(to_sell, volume)
                self.sell(bid_price, qty)
                to_sell -= qty
                print(f"SELL triggered at {bid_price} (True value: {true_value}, threshold: {threshold})")
...
class SquidStrategy(meanReversionStrategy):
