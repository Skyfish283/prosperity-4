...
class MacaronsStrategy(Strategy):
    def __init__(self, symbol: str, limit: int):
        super().__init__(symbol, limit)
        self.all_past_data: Dict[str, List[float]] = {}
        self.max_sample_size = 100
        self.conversion_limit = 10

    def load(self, data: Dict[str, List[float]]) -> None:
        self.all_past_data = data

    def save(self):
        return self.all_past_data
    
    def calc_sun_velocity(self, state: TradingState, periods_back: int) -> float:
        # Calculate the velocity of sunlight index
        periods_back = max(0, periods_back) # ensures periods_back is non-negative
        past_sunlight_data = self.all_past_data.get('sunlightIndex', [])
        if len(past_sunlight_data) < periods_back + 1:
            return 0.0
        return (past_sunlight_data[-(1 + periods_back)] - 
                past_sunlight_data[-(2 + periods_back)]) / 2
    ...
    def MACD(self, state: TradingState, ema_fast: int=12, 
        ema_slow: int=26, ema_sig: int=9) -> MACDsignals:
        past_data = self.all_past_data.get(self.symbol, [])
        # signal-line crossover, zero crossover and divergence respectively
        signals : MACDsignals = [0, 0, 0]
        if past_data is None or len(past_data) < (ema_fast + ema_slow + 1):
            return signals
    ...
    def act(self, state: TradingState) -> None:
        curr_sunlight = state.observations.conversionObservations[self.symbol].sunlightIndex
        csi = 45.0
        if curr_sunlight < csi: # sunlight below critical threshold
            if self.calc_sun_velocity(state, 1) == 0 and self.calc_sun_velocity(state, 2) == 0:
                # sunlight is constant, somewhat long macarons
                self.force_buy(state, 3)
                self.conversions = -3
            elif self.calc_sun_velocity(state, 1) <= 0 and self.calc_sun_velocity(state, 2) <= 0:
                # sunlight is nonincreasing, long macarons
                self.force_buy(state, 5)
                self.conversions = -5
            elif self.calc_sun_velocity(state, 1) >= 0 and self.calc_sun_velocity(state, 2) >= 0:
                # sunlight is nondecreasing, short macarons
                self.force_sell(state, 5)
                self.conversions = 5
            elif self.calc_sun_velocity(state, 1) <= 0 and self.calc_sun_velocity(state, 2) > 0:
                # potential local max, somewhat long macarons
                self.force_buy(state, 3)
                self.conversions = -3
            elif self.calc_sun_velocity(state, 1) >= 0 and self.calc_sun_velocity(state, 2) < 0:
                # potential local min, somewhat short macarons
                self.force_sell(state, 3)
                self.conversions = 3
        else:
            # sunlight above critical threshold, employ MACD
            MACD_signals = self.MACD(state)
            pos_diff = self.signal_interpretation(state, MACD_signals)
            if pos_diff > 0:
                true_pos_diff = min(pos_diff, self.limit - state.position.get(self.symbol, 0))
                self.force_buy(state, true_pos_diff)
                self.conversions = -true_pos_diff
            elif pos_diff < 0:
                true_pos_diff = min(-pos_diff, self.limit + state.position.get(self.symbol, 0))
                self.force_sell(state, true_pos_diff)
                self.conversions = true_pos_diff

        # Update past data
        curr_price = (state.observations.conversionObservations[self.symbol].bidPrice + state.observations.conversionObservations[self.symbol].askPrice) / 2
        curr_sunlight = state.observations.conversionObservations[self.symbol].sunlightIndex
        if len(self.all_past_data) > self.max_sample_size:
            self.all_past_data[self.symbol] = self.all_past_data[self.symbol][1:]
            self.all_past_data[self.symbol].append(curr_price)
            self.all_past_data['sunlightIndex'] = self.all_past_data['sunlightIndex'][1:]
            self.all_past_data['sunlightIndex'].append(curr_sunlight)
        elif len(self.all_past_data) == 0:
            self.all_past_data[self.symbol] = [curr_price]
            self.all_past_data['sunlightIndex'] = [curr_sunlight]
        else:
            self.all_past_data[self.symbol].append(curr_price)
            self.all_past_data['sunlightIndex'].append(curr_sunlight)
        return
    ...
    def __init__(self) -> None:
        ...
        self.strategies = {
            ...
            for symbol, clazz in {
                ...
                "MAGNIFICENT_MACARONS": MacaronsStrategy,
            }.items()
        }
